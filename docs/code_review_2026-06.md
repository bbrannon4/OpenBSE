...
| CR-4 8.4k-line main.rs mixes dispatcher + builders | **FIXED (relocated)** — the seven per-system-type signal builders (`build_psz/crac/crah/doas/fcu/vav/dual_duct_signals`) plus `LoopInfo`/`CoilRole`/`ZoneOaData`/`HvacMode`/`hvac_mode` moved out of `main.rs` (8.3k → 6.8k lines) into a new `openbse-airloop` crate. Note: the original target `openbse-controls` was not viable — `openbse-io` already depends on `openbse-controls`, and the builders consume `openbse_io::input` enums, so putting them in controls would cycle (io → controls → io). The new crate sits above io in the layering (depends on io/components/envelope/core/psychrometrics) and is separately testable. `build_loop_infos` stays in cli (it calls the io-private `compute_oa_fraction`); `LoopInfo`/`ZoneOaData` fields are now `pub`. Pure refactor: A/B vs the CR-5 HEAD baseline is bit-identical on 600/900/CE100 and single_family. Follow-up: the builders still take many per-zone maps by arg (crate-level `allow(too_many_arguments)`); a context struct is the natural next cleanup. |
| CR-7 duplicated EquipmentInput → ComponentKind match | **FIXED** — added canonical `EquipmentInput::component_kind()` / `PlantEquipmentInput::component_kind()` (plus `name()`) methods on the input enums in `io/input.rs` as the single source of truth for the energy-accounting kind mapping. The three identical copies of the match in `main.rs` (`build_loop_infos` `component_kinds`, the `comp_kind_map` air + plant loops) now call these methods; two pure name-extraction matches (`component_names`, the CRAC/CRAH submeter override) collapse to `.name()`. The accounting-specific overrides (CRAC/CRAH, terminal→HeatingCoil, exhaust-fan, DHW, VRF) intentionally stay in `main.rs` on top of the base mapping — the methods document that the accounting kind differs from a component's physical `component_kind()` in a few cases (e.g. WSHP metered as CoolingCoil), which is why graph-derived kinds were not used. A/B bit-identical on CE100, single_family, and the VRF / GSHP / CHW-plant / CRAC-CRAH-datacenter examples (results + summary). Note: chose enum methods over the review's "ask the graph" suggestion because the map is an accounting concern distinct from physical kind. |
| CR-8 detailed_outputs() per-timestep allocation | **FIXED** — added a visitor `report_outputs(&self, out: &mut dyn FnMut(&str, f64))` to the `AirComponent`/`PlantComponent` traits; all 21 impls (19 components + 2 cosim proxies) now push key/value pairs through `out(...)` instead of allocating a fresh `HashMap` per component per timestep. The 3 hot-path call sites in `main.rs` insert directly into the existing output map. `detailed_outputs()` is kept as a default trait method that materializes the visitor into a map, so every existing component unit test (which asserts exact keys/values) still passes unchanged — that is the cross-impl verification backstop. A/B vs baseline bit-identical on 600/900/CE100 and single_family. |
| CR-5 stale outputs under PLR scaling | **FIXED** — the system-level PLR block in `main.rs` now scales the extensive `detailed_outputs()` rate/flow keys (`sensible_load`, `latent_load`, `total_load`, `cycling_loss`, `sensible_cooling_rate`, `latent_cooling_rate`, `heat_to_air`, `shaft_power`, `conduction_loss`, `leakage_loss`, `heat_transferred`, `source_mass_flow`, `water_mass_flow`) by the same factor as `thermal_output`/`mass_flow`, in both the cycling and no-load-off branches. Intensive states (temps, humidity, enthalpy, `plr`, `rtf`, COP/efficiency, `pressure_rise`, `effectiveness`, water in/out temps) are left pre-PLR by design and documented as on-cycle instantaneous values; system PLR is reported separately as `__loop_plr__`. A/B vs HEAD baseline: 600/900 bit-identical; CE100/SF changed only the listed rate columns (`total_load`/`\|thermal_output\|` ratio went from 1.34 → 1.000), no energy totals moved. |
