...
pub fn calculate_ctf(layers: &[ResolvedLayer], dt: f64) -> CtfCoefficients {
    ...
    let total_c: f64 = layers.iter()
        .map(|l| l.density * l.specific_heat * l.thickness)
        .sum();

    // Very low thermal mass or no layers: steady-state CTF
    if total_c < 1.0 || layers.is_empty() || total_r <= 0.0 {
    ...
    // Compute nodes per layer and build LayerProps
    let layer_props: Vec<LayerProps> = layers.iter().map(|l| {
    ...
    // Total state-space nodes (rcmax): sum of layer nodes, minus (num_layers - 1)
    // for shared interface nodes, minus 2 for the two surface nodes.
    // Actually: each layer has `nodes` internal divisions, giving (nodes - 1) interior nodes.
    // For a single layer: rcmax = nodes - 1
    // For multiple layers: rcmax = sum(nodes) - (num_layers - 1) - 1
    // Wait — E+ convention: rcmax = total nodes - 1 where the -1 accounts for one boundary.
    // Let me follow E+ exactly:
    //   Each layer contributes `nodes` intervals (nodes+1 node points).
    //   Adjacent layers share boundary nodes.
    //   Total node points = sum(nodes) + 1  [for single layer, nodes + 1 points]
    //   Surface nodes (first and last) are NOT part of state space.
    //   So rcmax = total_node_points - 2 = sum(nodes) - 1
    ...
    let (a_mat, b_vec, c_vec, d_vec) = build_state_space(&layer_props, rcmax);

    // Compute CTF from state-space
    compute_ctf_from_state_space(&a_mat, &b_vec, &c_vec, &d_vec, rcmax, dt)
}
...
pub fn calculate_ctf_simple(u_factor: f64, thermal_capacity: f64, dt: f64, mass_outside: bool) -> CtfCoefficients {
    ...
    // Determine if a 2-layer (insulation + mass) model is needed.
    // Use mass properties: k=1.0 W/mK (typical concrete/masonry range 0.5-1.8 W/mK)
    let k_mass = 1.0;
    let rho_mass = 2000.0;
    let cp_mass = 1000.0;
    ...
    if r_total > 2.0 * r_mass {
        ...
        let insul = ResolvedLayer {
            conductivity: actual_k_insul,
            density: rho_insul,
            specific_heat: cp_insul,
            thickness: t_insul,
        };
        let mass = ResolvedLayer {
            conductivity: k_mass,
            density: rho_mass,
        ...
        if mass_outside {
            ...
            let mass_layer = ResolvedLayer {
                conductivity: k_mass,
                density: rho_mass,
                specific_heat: cp_mass,
                thickness: t_mass_adj,
            };
            let insul_layer = ResolvedLayer {
                conductivity: actual_k_insul_adj,
                density: rho_insul,
                specific_heat: cp_insul,
                thickness: t_insul_adj,
            };
            let finish_layer = ResolvedLayer {
                conductivity: k_fin,
                density: rho_fin,
                specific_heat: cp_fin,
                thickness: t_fin,
            };
...
fn build_state_space(
    ...
    for i in 0..n {
        ...
        // Left coupling (node i-1 → node i)
        let k_l = nodes[i].k_left;
        let dx_l = nodes[i].dx_left;
        let cond_left = k_l / dx_l;

        // Right coupling (node i → node i+1)
        let k_r = nodes[i].k_right;
        let dx_r = nodes[i].dx_right;
        let cond_right = k_r / dx_r;
    ...
    // Outside input → node 0
    b[0 * 2 + 0] = first_layer.k / first_layer.dx / cap_0;
    // Inside input → node n-1
    b[(n - 1) * 2 + 1] = last_layer.k / last_layer.dx / cap_n;

    // C matrix: fluxes at surfaces
    // Outside flux = -k_out/dx_out * (T_node0 - T_surface_out)
    // → C contribution from node 0: -k_out/dx_out (sign: E+ convention)
    c[0 * n + 0] = -first_layer.k / first_layer.dx;       // outside flux, node 0
    c[1 * n + (n - 1)] = last_layer.k / last_layer.dx;     // inside flux, node n-1

    // D matrix: direct feedthrough (surface temp → surface flux)
    d[0 * 2 + 0] = first_layer.k / first_layer.dx;    // outside temp → outside flux
    d[1 * 2 + 1] = -last_layer.k / last_layer.dx;     // inside temp → inside flux
