...
mod ctf_compare {
    ...
    fn test_ctf_compare_wall() {
        ...
        let k_fin: f64 = 0.14; let rho_fin: f64 = 530.0; let cp_fin: f64 = 900.0; let t_fin: f64 = 0.009;
        let cap_fin: f64 = rho_fin * cp_fin * t_fin;
        let k_mass: f64 = 0.51; let rho_mass: f64 = 1400.0; let cp_mass: f64 = 1000.0;
        ...
        let actual_k_insul: f64 = if t_insul < t_insul_raw { t_insul / r_insul } else { k_insul };
        ...
        println!("Current Z[0]:  {:.4} (ratio: {:.4})", our_ctf.z[0], our_ctf.z[0] / ep_ctf.z[0]);
        println!("Proposed Z[0]: {:.4} (ratio: {:.4})", proposed_ctf.z[0], proposed_ctf.z[0] / ep_ctf.z[0]);

        println!("\n=== WALL X[0] (exterior self-response) ===");
        println!("E+ X[0]:       {:.4}", ep_ctf.x[0]);
        println!("Current X[0]:  {:.4} (ratio: {:.4})", our_ctf.x[0], our_ctf.x[0] / ep_ctf.x[0]);
        println!("Proposed X[0]: {:.4} (ratio: {:.4})", proposed_ctf.x[0], proposed_ctf.x[0] / ep_ctf.x[0]);
        ...
        let our_floor_ctf = calculate_ctf_simple(0.0396, 112000.0, dt, false, Some(1.13), Some(1400.0));

        println!("E+ Floor Z[0]:  {:.4}", ep_floor_ctf.z[0]);
        println!("Our Floor Z[0]: {:.4} (ratio: {:.4})", our_floor_ctf.z[0], our_floor_ctf.z[0] / ep_floor_ctf.z[0]);
    }
}
